...
import { RawCombinedCredit, RawMovieCredit, RawTvShowCredit } from "@/lib/tmdb/models";
...
export const PersonCreditsTable: React.FC<PersonCreditsTableProps> = ({ credits, department }) => {
  const [value, setValue] = React.useState<string>("all");

  const sort = (a: RawCombinedCredit, b: RawCombinedCredit) => {
    const aDate = new Date(a.media_type === "tv" ? a.first_air_date : a.release_date);
    const bDate = new Date(b.media_type === "tv" ? b.first_air_date : b.release_date);
            ...
            <TableBody>
              ...
              {sortedList.filter(filter)?.map((credit) =>
                credit.media_type === "tv" ? (
                  <CommandItem asChild key={credit.credit_id} value={credit.name}>
                    <CreditsTableTvItem {...credit} />
                  </CommandItem>
                ) : (
                  <CommandItem asChild key={credit.credit_id} value={credit.title}>
...
const CreditsTableMovieItem = React.forwardRef<HTMLTableRowElement, RawMovieCredit>(
  ({ id, release_date, title, character, job }, ref) => (
    <TableRow ref={ref}>
      ...
      <TableCell>
        ...
        {(character || job) && <p className="text-muted-foreground">as {character || job}</p>}
      </TableCell>
    </TableRow>
  )
);
...
const CreditsTableTvItem = React.forwardRef<HTMLTableRowElement, RawTvShowCredit>(
