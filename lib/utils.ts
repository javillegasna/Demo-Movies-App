...
export function getPersonHighlights(
  ...
  const list = isActing ? sortByVoteScore(cast.filter(isRemarkable)) : sortByVoteScore(crew);

  const highlights = getUniqueItems(list).slice(0, count);

  const hero = getRandomItems(highlights.filter((item) => item.backdrop_path))[0];
...
export function filterByDepartment(list: RawCombinedCredit[], department: string) {
  return list.filter((item) => item.department === department);
}

export function sortByReleaseDate(list: Movie[], order: "asc" | "desc" = "asc") {
...
export function filterDiscoverParams(params?: Record<string, string>): Record<string, string> {
  return Object.fromEntries(
    Object.entries(params ?? {}).filter(([key]) => availableParams.includes(key))
  );
}
