...
export const DiscoverFilterVoteAverage: React.FC<DiscoverFilterVoteAverageProps> = ({
  value: initialValue,
  onChange,
}) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <Slider min={0} max={10} step={1} value={[value]} onValueChange={handleValueChange} />

      <div className="mt-4 flex justify-between border-t">
        {Array.from({ length: 11 }, (_, i) => (
          <div key={i} className="relative pt-2">
            <span className={cn("text-[9px]", value !== i && "text-muted-foreground")}>{i}</span>
